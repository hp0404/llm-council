...
from .models import format_models_for_ui, get_available_models
...
def check_rate_limit(identifier: str) -> bool:
    ...
    _auth_attempts[identifier] = [
        t for t in _auth_attempts[identifier] if now - t < _AUTH_WINDOW_SECONDS
    ]
...
async def verify_token(request: Request, x_auth_token: str | None = Header(None)):
...
async def list_models(
    request: Request = None, authenticated: bool = Depends(verify_token)
):
    """
...
async def list_conversations(
    request: Request, authenticated: bool = Depends(verify_token)
):
        ...
        try:
            ...
            if is_first_message:
                title_task = asyncio.create_task(
                    generate_conversation_title(msg_request.content)
                )

            # Stage 1: Collect responses
            yield f"data: {json.dumps({'type': 'stage1_start'})}\n\n"
            stage1_results = await stage1_collect_responses(
                msg_request.content, council_models=model_config["council_models"]
            )
